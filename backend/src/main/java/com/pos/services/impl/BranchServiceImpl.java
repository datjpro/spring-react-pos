...
public class BranchServiceImpl implements BranchService {
    ...
    private BranchEntity find(Long id) {
        return branchRepository.findByIdAndActiveTrue(id)
                .orElseThrow(() -> new ResourceNotFoundException("Branch not found"));
    }

    private void apply(BranchEntity e, BranchRequest r) {
        e.setCode(r.code());
        e.setName(r.name());
        e.setAddress(r.address());
    }

    private BranchResponse map(BranchEntity e) {
        return new BranchResponse(e.getId(), e.getCode(), e.getName(), e.getAddress(), e.isActive());
    }
}
